...
multitail \
  ...
  -l "bash -c 'while true; do echo; echo \"[$(date \"+%H:%M:%S\")] PROCESS STATUS\"; ps aux | grep train_classification_stage | grep -v grep | head -3 | awk \"{printf \\\"%s %s CPU:%s MEM:%s CMD:%s\\n\\\", \\\$2, \\\$8, \\\$3, \\\$4, \\\$11}\"; echo; sleep 3; done'" \
  -l "bash -c 'while true; do 
      echo \"[$(date \"+%H:%M:%S\")] TRAINING PROGRESS\"; 
      latest_report=\$(ls -t /home/max16/pillsnap/artifacts/reports/training_progress_reports/*.json 2>/dev/null | head -1);
...
try:
    ...
    print(f'목표 달성: {data[\\\"validation_results\\\"][\\\"stage1_completed\\\"]}')
