#!/bin/bash
# Stage 2 학습 + 통합 실시간 모니터링 스크립트

echo "🚀 PillSnap Stage 2 학습 + 모니터링 시작"
echo "=============================================="

# 로그 디렉토리 생성
LOG_DIR="/tmp/pillsnap_training_stage2"
mkdir -p "$LOG_DIR"

# 백그라운드로 학습 시작
echo "📚 백그라운드 학습 시작..."
nohup ./scripts/core/python_safe.sh -m src.training.train_classification_stage --manifest artifacts/stage2/manifest_ssd.csv --stage 2 --epochs 30 --batch-size 112 > "$LOG_DIR/training.log" 2>&1 &
TRAINING_PID=$!
...
multitail \
  -l "bash -c 'while true; do 
    ...
    tail -10 $LOG_DIR/training.log 2>/dev/null | grep -E \"(Epoch|Batch|Loss|Acc|목표|달성|완료|INFO)\" | tail -5;
    echo; 
    sleep 2; 
  done'" \
  -l "bash -c 'while true; do 
  ...
  -l "bash -c 'while true; do 
    echo \"[$(date \"+%H:%M:%S\")] 📈 STAGE 2 PROGRESS\"; 
    # 최신 정확도 추출
    ...
    # 클래스 수 및 샘플 수
    echo \"클래스 수: 237개 (Stage 2)\";
    echo \"훈련 샘플: 18,960개 | 검증 샘플: 4,740개\";
