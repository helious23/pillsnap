...
# 사용법: bash scripts/core/setup_venv.sh
...
if [[ "$VENV_EXISTS" == "true" ]]; then
    # Python 버전 확인
    PYTHON_VERSION=$(./scripts/core/python_safe.sh --version 2>/dev/null | grep -o "[0-9]\+\.[0-9]\+\.[0-9]\+" || echo "unknown")
    ...
    PYTORCH_CUDA_CHECK=$(./scripts/core/python_safe.sh -c "
import torch
...
" 2>/dev/null || echo "ERROR")
    ...
    # GPU 하드웨어 확인
    echo "  GPU 하드웨어 확인 중..."
    GPU_INFO=$(./scripts/core/python_safe.sh -c "
import torch
...
if [[ "$VENV_EXISTS" == "true" ]]; then
    echo "  import 테스트 중..."
    IMPORT_TEST=$(./scripts/core/python_safe.sh -c "
try:
...
echo "./scripts/core/python_safe.sh tests/test_stage1_real_image.py"
echo ""
echo "# 실제 학습 시작"
echo "./scripts/core/python_safe.sh -m src.training.train_classification_stage --stage 1 --epochs 10"
echo ""
echo "# 성능 평가"
echo "./scripts/core/python_safe.sh -m src.evaluation.evaluate_pipeline_end_to_end --stage 1"
echo ""
echo -e "${BLUE}=================================================${NC}"
