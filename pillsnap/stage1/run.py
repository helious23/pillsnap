    ...
    try:
        ...
        with Progress(
            ...
            if not md_report_path.exists():
                try:
                    _generate_markdown_report(md_report_path, stats, report, df_processed)
        ...
        table.add_row("스캔된 파일", f"{stats.get('total_images', 0):,} 이미지, {stats.get('total_labels', 0):,} 라벨")
        ...
        # 최종 결과
        if report.passed:
            console.print(Panel.fit("🎉 Stage 1 파이프라인 성공적으로 완료!", style="bold green"))
            
            # 다음 단계 안내
            console.print("\n🔗 다음 단계:")
            console.print("  python -m src.train --mode single --epochs 10 --batch-size 32")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--limit", type=int, default=400,
                       help="스캔 제한 (0=전체, 기본 400 안전 권장)")
    parser.add_argument("--manifest", type=str, default="artifacts/manifest_stage1.csv",
                       help="출력 매니페스트 경로")
